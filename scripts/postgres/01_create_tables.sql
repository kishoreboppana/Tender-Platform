-- TMS table definitions (mirror of Flyway V1__initial_tms_schema.sql)
-- Use when you want schema without running the Spring Boot app.
-- Requires database tms and user tms.

CREATE SCHEMA IF NOT EXISTS tms;

CREATE TABLE IF NOT EXISTS tms.tenant (
    id              UUID PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_customer_tenant ON tms.customer(tenant_id);

CREATE TABLE IF NOT EXISTS tms.business_sequence (
    tenant_id       UUID         NOT NULL REFERENCES tms.tenant(id),
...
CREATE TABLE IF NOT EXISTS tms.tender (
    id                  BIGSERIAL PRIMARY KEY,
    tenant_id           UUID         NOT NULL REFERENCES tms.tenant(id),
...
CREATE INDEX IF NOT EXISTS idx_tender_tenant ON tms.tender(tenant_id);
CREATE INDEX IF NOT EXISTS idx_tender_status ON tms.tender(status);
CREATE INDEX IF NOT EXISTS idx_tender_closing ON tms.tender(closing_at);

CREATE TABLE IF NOT EXISTS tms.audit_event (
    id              BIGSERIAL PRIMARY KEY,
    tenant_id       UUID         NOT NULL REFERENCES tms.tenant(id),
...
CREATE INDEX IF NOT EXISTS idx_audit_tenant ON tms.audit_event(tenant_id);
CREATE INDEX IF NOT EXISTS idx_audit_created ON tms.audit_event(created_at);
...
CREATE INDEX IF NOT EXISTS idx_app_user_tenant ON tms.app_user(tenant_id);
